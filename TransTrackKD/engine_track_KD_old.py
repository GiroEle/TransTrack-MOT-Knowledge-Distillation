...
def train_one_epoch(model: torch.nn.Module, model_large: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for samples, targets in metric_logger.log_every(data_loader, 10, header):
        ...
        if tracker is not None:
            if phase != 'train':
                ...
                assert samples.shape[0] == 1, "Now only support inference of batchsize 1." 
            #frame_id = targets[0].get("frame_id", None)
            frame_id = targets.get("frame_id", None)
