...
#     metric_logger.add_meter('class_error', utils.SmoothedValue(window_size=1, fmt='{value:.2f}'))
    ...
    order_check = numpy.empty((0,2)).astype(int)

    res_tracks = dict()
    pre_embed = None
    for samples, targets in metric_logger.log_every(data_loader, 10, header):
        #print(targets[0]['image_id'].item(),targets[0]['frame_id'].item())
        order_check =numpy.append(order_check,[[targets[0]['image_id'].item(),targets[0]['frame_id'].item()]],axis=0 )
...
#         metric_logger.update(class_error=loss_dict_reduced['class_error'])
    ...
    print(order_check)
    numpy.savetxt("ordercheck.txt",order_check, delimiter=',')
